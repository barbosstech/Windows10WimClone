...
@if "%~x1" == "C:\Imagens\PRIOSysPrep.wim" (GOTO WIM)
@if "%~x1" == "D:\Imagens\PRIOSysPrep.wim" (GOTO WIM)
@if "%~x1" == "E:\Imagens\PRIOSysPrep.wim" (GOTO WIM)
@if "%~x1" == "F:\Imagens\PRIOSysPrep.wim" (GOTO WIM)
@if "%~x1" == ".ffu" (GOTO FFU)
@echo *********************************************************************
...
@echo     expandir a particacao apos o clone digite N). 
...
@echo Esta imagem contem atributos estendidos?
@echo    (Se nao tem certeza, digite N).
@SET /P EA=(Y or N):
@if %EA%.==y. set EA=Y
